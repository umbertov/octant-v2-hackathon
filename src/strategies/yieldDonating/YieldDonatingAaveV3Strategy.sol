...
interface IPool {
    ...
    function getReserveAToken(address) external returns (address);
}
 ...
 */
contract YieldDonatingAaveV3Strategy is BaseStrategy {
        ...
        )
    {
        ...
        address _aToken = lendingPool.getReserveAToken(_asset);
    ...
    function _harvestAndReport() internal override returns (uint256 _totalAssets) {
        ...
        // Calculate aToken balance (assets earning yield in Aave)
        uint256 aTokenAssets = aToken.balanceOf(address(this));

        // If there are idle assets, deploy them to the lending pool
        if (idleAssets > 0) {
            lendingPool.supply(address(asset), idleAssets, address(this), 0);
        }

        // Update total assets under management. warning: calculation is made using pre-supply amounts
